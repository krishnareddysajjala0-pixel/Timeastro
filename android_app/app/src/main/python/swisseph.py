...
# Attempt to load shared C library libswisseph.so on Android
try:
    _lib = ctypes.CDLL("libswisseph.so")
except Exception:
    try:
        # Fallback for Windows or desktop testing if available
        dll_path = os.path.join(os.path.dirname(__file__), "libswisseph.dll")
        if os.path.exists(dll_path):
            _lib = ctypes.CDLL(dll_path)
    except Exception as e:
        print("[swisseph] Note: libswisseph native binary not loaded yet:", e)
